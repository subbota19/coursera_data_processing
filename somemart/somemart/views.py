...
class GetItemView(View):
    ...
    def get(self, request, item_id):
        if "HTTP_AUTHORIZATION" in request.META:
            user_name, password = base64.b64decode(request.META['HTTP_AUTHORIZATION'].split()[1]).decode().split(":")
            print(user_name, password)
            if list(User.objects.filter(first_name=user_name, password=password)):
                if Item.objects.filter(id=item_id):
                ...
                response['WWW-Authenticate'] = 'Basic realm=Authenticate Protected'
                return response
